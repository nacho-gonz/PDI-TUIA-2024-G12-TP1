...
def encontrar_mayor_componente_horizontal(image_binaria : np.ndarray) -> tuple[int, tuple[int,int]]:
    ...
    # Inicializar variables para almacenar el componente más ancho
    max_ancho = 0
...
def resolver_examenes() -> dict[str:[dict[str:str],dict[str:str],np.ndarray]]:
    """
    Esta función acepta como parametro el path de la carpeta con todas las imagenes de examanes a corregir y devuelve un diccionario
    con todos los examenes corregidos y la validación de los datos de encabezado.
    
    Parameters
    -----------
    directorio : string
        Path de la carpeta de las imagenes de los examenes.

    
    Returns
    -----------
    examenes : dictionary 
        Un diccionario donde la clave es cada examen y los valores son:
    ...
    ValueError : Si el directorio está vacío o no contiene exámenes válidos.
    """
    ...
    for i in range(1,num_elementos+1):
        imagenes.append(cv2.imread(f'./examenes/examen_{i}.png', cv2.IMREAD_GRAYSCALE)) #En este for recorremos todas las imagenes de una carpeta 
    ...
    for examen in imagenes:
        ...
        if (cant_caracteres_fecha-1) == 8:
            condicion_alumno['Date'] = 'OK'
        else:
            condicion_alumno['Date'] = 'MAL'
        
        if (cant_caracteres_clase-1) == 1:
...
while True:
    ...
    match opcion:
        case 1:
            imagen = cv2.imread('Imagen_con_detalles_escondidos.tif', cv2.IMREAD_GRAYSCALE)
            imagen_equalizada = funcion1_tp_imagen(imagen, 24)
